...
for x in range(len(newNames)):
	newNames[x] = str(x+1) +"-"+newNames[x]
	temp_list = newNames[x].split("-")
	origi_name = temp_list[1]
	shutil.move(path+"sorted/"+"full_sort/"+origi_name,path+"sorted/"+"full_sort/"+newNames[x] )
