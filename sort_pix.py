...
for image in block_imagery:
    ...
    if (block_imagery.index(image) == 0) or (
            image[:2] != block_imagery[block_imagery.index(image) - 1][:2]):
        group_num += 1
        #make a start_date string that tells you the youngest file in the group
        if (start_date + " (" + str(group_num) + ")" + " - " + current_pre
                not in file_dict.keys()) or (len(
                    file_dict["group" + str(group_num) + start_date]) == 0):
            start_date = ""
            temp_list = []
            temp_list = image.split("2", 1)
            start_date = "2" + temp_list[1]
            start_date = " " + start_date[0:4] + " " + start_date[4:6]
        file_dict[start_date + " (" + str(group_num) + ")" + " - " +
                  current_pre] = []
        file_dict[start_date + " (" + str(group_num) + ")" + " - " +
                  current_pre].append(image)
    elif (image.find("IS") != -1 and
          block_imagery[block_imagery.index(image) - 1].find("IS") != -1) or (
              image.find("IP") != -1 and
              block_imagery[block_imagery.index(image) - 1].find("IP") != -1):
        file_dict[start_date + " (" + str(group_num) + ")" + " - " +
                  current_pre].append(image)
